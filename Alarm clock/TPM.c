...
void PWM_Init()
{
	...
	TPM0->CONTROLS[0].CnSC = TPM_CnSC_MSB_MASK|TPM_CnSC_ELSA_MASK;	//	TPM0, kanał 0 tryb "Edge-aligned PWM Low-true pulses (set Output on match, clear Output on reload)"
	TPM0->CONTROLS[0].CnV = 0x0000;					// Wsółczynnik wypełnienia początkowo 0
	TPM0->SC |= TPM_SC_CMOD(1);							// Włacz licznik TPM0
}
